...
public class SLList {
    ...
    public void add(int index, int x) {
        IntListNode n = sentinel;
        if (index >= size) index = size;
        for (; index > 0; index--){
            n = n.next;
        }
        n.next = new IntListNode(x,n.next);
        size++;
    }
    ...
    //return the reversed first node
    public IntListNode reverseHelper(IntListNode L){
        if(L.next == sentinel){return L;}
        if(L.next.next == sentinel){
            IntListNode temp = new IntListNode(L.item, sentinel);
            L = L.next;
            L.next = temp;
            return L;
        }
        return reverseHelper(L.next);
        //IntListNode temp = reverseHelper(L.next);
        //while(temp.next != sentinel) temp = temp.next;
        //temp.next = L;
    }
/*
    //leap of faith
    public IntListNode lastNode(IntListNode L){
        if(L.next == sentinel){return L;}
        return (reverseHelper(L.next));
    }
