...
public class AugmentedStreetMapGraph extends StreetMapGraph {
    ...
    public AugmentedStreetMapGraph(String dbPath) {
        ...
        pointNodeMap = new HashMap<Point, Node>();
        for(Node n : nodes){
            if(neighbors(n.id()).size() > 0){
                double x = projectToX(n.lon(), n.lat());
                double y = projectToY(n.lon(), n.lat());
                Point temp = new Point(x, y);
                pointNodeMap.put(temp, n);
            }
        }
        List<Point> points = new ArrayList<>(pointNodeMap.keySet());
    ...
    public long closest(double lon, double lat) {
        ...
        return pointNodeMap.get(kd.nearest(x, y)).id();
    }
