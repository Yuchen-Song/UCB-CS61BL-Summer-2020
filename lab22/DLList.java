...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        ...
        for (Node ptr = sentinel.next; ptr != sentinel; ptr = ptr.next) {
            if (ptr.next == sentinel) {
            ...
            } else if (ptr.item.compareTo(item) <= 0 && ptr.next.item.compareTo(item) > 0) {
                Node newNode = new Node(item);
                ptr.next.prev = newNode;
                newNode.next = ptr.next;
                ptr.next = newNode;
                newNode.prev = ptr;
                return;
            } else if (ptr.item.compareTo(item) > 0){
                Node newNode = new Node(item);
                ptr.prev.next = newNode;
                newNode.prev = ptr.prev;
                ptr.prev = newNode;
    ...
    public DLList<T> quicksort() {
        ...
        int flag = 0;
        //build these three dllist
        for (Node ptr = sentinel.next; ptr!=sentinel; ptr=ptr.next) {
            if (ptr.item.compareTo(pivot) < 0) {
                smallElements.addLast(ptr.item);
            } else if (flag == 0 && ptr.item.compareTo(pivot) == 0) {
        ...
        DLList<T> toReturn = smallElements.quicksort();
