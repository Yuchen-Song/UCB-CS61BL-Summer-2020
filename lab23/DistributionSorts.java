...
public class DistributionSorts {
    ...
    private static void countingSortOnDigit(int[] arr, int digit) {
        int[] arrOnDigit = new int[arr.length];
        for (int i=0; i<arr.length; i++) {
            arrOnDigit[i] = (int) (arr[i] / Math.pow(10,digit) % 10);
        }
        countingSort(arrOnDigit);
        System.arraycopy(arrOnDigit,0,arr,0,arr.length);
    }
